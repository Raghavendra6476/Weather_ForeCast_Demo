...
public class NVActivityIndicatorPresenter {
    ...
    private func show(with activityData: ActivityData)
    {
        ...
        let size = activityData.message?.size(withAttributes: fontAttributes)
        
        var svWidth = activityContainer.frame.size.width / 3
        
        if (size?.width)! > svWidth {
            
            if (size?.width)! < SCREEN_WIDTH! - 60 {
                
                svWidth = (size?.width)! + 15
                
            } else {
                
                svWidth = SCREEN_WIDTH! - 60
            }
        ...
        subActivityContainer.backgroundColor = APP_COLOR//UIColor(netHex:0xF7323F)//b88955
...
//        UIApplication.shared.keyWindow!.addSubview(activityContainer)
        WINDOW_DELEGATE??.addSubview(activityContainer)
    }
    
    private func hide()
    {
        for item in (WINDOW_DELEGATE??.subviews)!
